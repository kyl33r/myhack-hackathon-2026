...
import { submitStartupProfile, getMatches } from '../../services/api'
...
export default function ProfileForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitError(null)
    try {
      const { startupId } = await submitStartupProfile(form)
      const matches: MatchResponse = await getMatches(startupId)
      navigate('/startup/results', {
        state: { startupId, startupName: form.startupName, matches },
      })
    } catch (err: unknown) {
  ...
  return (
    <>
      ...
      <LoadingOverlay
        ...
        sub="Analysing mentors, programmes, and partners"
      />
    </>
  )
}
