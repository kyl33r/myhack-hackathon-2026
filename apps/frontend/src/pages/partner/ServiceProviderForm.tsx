...
export default function ServiceProviderForm() {
  ...
  const [form, setForm] = useState<FormState>({
    ...
    serviceType: '', whatOffer: '', pricingModel: '', stages: [],
  })
  ...
  function handleStageCheckbox(value: string, checked: boolean) {
    setForm(f => ({
      ...f,
      stages: checked ? [...f.stages, value] : f.stages.filter(v => v !== value),
    }))
  }
  ...
  function validate(): boolean {
    ...
    if (!form.contactEmail.trim() || !form.contactEmail.includes('@')) e.contactEmail = 'A valid email is required.'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitError(null)
    try {
      await submitPartnerProfile({ ...form, partnerType: 'service_provider' })
      setSubmitted(true)
    } catch (err: unknown) {
      ...
      <main className="container">
        ...
        <div className="card form-card">
          <div className="form-type-badge">
            <span className="actor-tag tag-service">Service Provider</span>
          </div>
